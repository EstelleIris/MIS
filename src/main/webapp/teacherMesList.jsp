...
<head>
    ...
    <script>
        function deleteTeacher(no) {
            if (confirm("确定删除?")) {
                location.href = "deleteServlet?type=teacher&cj_Tno02=" + no;
            }
        }
    </script>
</head>
    ...
    <div class="row">
        ...
        <div class="col-md-8 col-md-offset-1">
            <ol class="breadcrumb">
                ...
                <li class="active"><label class="breadcrumbFont2">教师管理</label></li>
            </ol>
        </div>
        <!--信息表格-->
        <div class="col-md-8 col-md-offset-1 table-responsive">
            <form class="form-inline" action="queryServlet?type=teacher&role=${param.role}" method="post">
                <div class="form-group">
                    <label for="inputNo">工号</label>
                    <input type="text" class="form-control" value="${condition.cj_Tno02[0]}" name="cj_Tno02"
                           id="inputNo">
                </div>
                <div class="form-group">
                    <label for="inputName">姓名</label>
                    <input type="text" class="form-control" value="${condition.cj_Tname02[0]}" name="cj_Tname02"
                           id="inputName">
                </div>
                <button type="submit" class="btn btn-default btn-success">查询</button>
                <a href="add.jsp?type=teacher&role=${param.role}" class="btn btn-default btn-info" role="button"
                   style="margin-left: 100px">添加教师</a>
                <a href="import.jsp?type=teacher&role=${param.role}" class="btn btn-default btn-info" role="button">导入教师数据</a>
            </form>
                    ...
                    <c:forEach var="teacher" items="${requestScope.teacherList}"
                        ...
                        <td>
                            <a class="btn btn-default btn-sm btn-info"
                               href="modifyTeacher.jsp?no=${teacher.no}&name=${teacher.name}&sex=${teacher.sex}&age=${teacher.age}&title=${teacher.title}&phone=${teacher.phone}">
                                <span class="glyphicon glyphicon-edit"></span></a>
                            <a class="btn btn-default btn-sm btn-info"
                               href="javascript:deleteTeacher('${teacher.no}');">
